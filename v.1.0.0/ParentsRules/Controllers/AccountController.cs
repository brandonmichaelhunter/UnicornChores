    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Check to see if the user is a parent or a child.
                    var currentUsr = _context.AccountUsers.Where(a => a.UserName.ToLower() == model.UserName.ToLower()).FirstOrDefault();
                    if(currentUsr != null)
                    {
                        if(currentUsr.IsChild == "0")
                        {
                        ...
                        {
                            _logger.LogInformation("User logged in.");
                            return RedirectToAction("Index", "ChildrenWorks");
                        }
                    }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    /* Create user account. */
                    var user = new ApplicationUser { UserName = model.Username, Email = model.Email, FirstName = model.FirstName, MiddleName = model.MiddleName, LastName = model.LastName,IsChild ="0", PhoneNumber = model.PhoneNumber };
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (model.IsLinkUserRegistration)
                        {
                            ...
                            CreateRelationship(model.RequestorID, user.Id);

                            /* Confirm user's request. */
                            ConfirmFriendsUserRequest(model.RegistrationCode);
                        }
            ...
            {
                _logger.LogError(string.Format("{0} - Error Message: {1}", System.Reflection.MethodBase.GetCurrentMethod(), ex.Message));
                throw new ApplicationException(string.Format("{0} - Error Message: {1}", System.Reflection.MethodBase.GetCurrentMethod(), ex.Message));
                return RedirectToAction("Index", "StatusCode", 500);
            }
        }
        ...
        private bool CreateRelationship(string RequestorID, string FriendID)
        {
            try
            {
                ...
                var PrimaryUserAssociation = new AccountAssociations() { PrimaryUserID = RequestorID, AssociatedUserID = FriendID };
                var SecondaryUserAssociation = new AccountAssociations() { PrimaryUserID = FriendID, AssociatedUserID = RequestorID };
        ...
        public async Task<IActionResult> RegisterAssociatedUser(RegisterViewModel model, string returnUrl = null)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    /* Create user account. */
                    var user = new ApplicationUser { UserName = model.Username, Email = model.Email, FirstName = model.FirstName, MiddleName = model.MiddleName, LastName = model.LastName, PhoneNumber = model.PhoneNumber };
                    var result = await _userManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("User created a new account with password.");
                        ...
                        List<AccountAssociations> userAssociations = new List<AccountAssociations>();
                        var PrimaryUserAssociation = new AccountAssociations() { PrimaryUserID = requestorID, AssociatedUserID = requesteID };
                        var SecondaryUserAssociation = new AccountAssociations() { PrimaryUserID = requesteID, AssociatedUserID = requestorID };
                        _context.AccountAssociations.Add(PrimaryUserAssociation);
                        _context.AccountAssociations.Add(SecondaryUserAssociation);
                        _context.SaveChanges();
